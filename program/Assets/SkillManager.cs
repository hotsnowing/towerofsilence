...
public class SkillManager : MonoBehaviour
{
    ...
    */
    ...
    {
        //skill_Id, skill_Icon, skill_Name, skill_Cost [0레벨 ~ 5레벨], skill_Content
        basicSkillDatas = new BasicSkillData[40];
        basicSkillDatas[0] = new BasicSkillData(0, skill_Icons[0], "무기 강타", new int[6] { 0, 1, 2, 4, 6, 8 }, " 가장 앞의 적을 가격하여 피해를 입힙니다/3레벨 기정상태 추가 ", true);
    }
    public BasicSkillData GetBasicSkillData(SkillData skillData)
    {
        foreach(BasicSkillData temp in basicSkillDatas)
        {
            if(temp.skill_Id == skillData.skill_Id) return temp;
        }
        Debug.LogError("CanFindSkill : skillManager.GetSkillCost");
        return null;
    }
        ...
        switch (skillData.skill_Id) 
        {
            case 0:
                ...
                if (pT && eT)
                {
                    ...
                    PlayerSkill1(pT, eT, skillData.skill_Level);
                    return true;
                }
                break;
        }
